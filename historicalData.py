...
class historicalData():
    ...
    def crawlFR(self):
        ...
        while (go):
            ...
            if self.coin == "BTC":
                url = "https://www.bitmex.com/api/v1/funding?count=500&start=" + str(
                    500 * i
            ...
            for index in range(0, len(fundingRate)):
                historicalData.fundingRateHistory[
                    fundingRate[index]['timestamp']
                ] = fundingRate[index]['fundingRate'] * 100
                if (
                    fundingRate[index]['timestamp'] ==
        ...
        # p.m.20:00 to a.m. 00:00
            ...
            tmpStr = 'T04:00:00.000Z'
            utcNow = datetime.datetime.strptime(str(utcNow)[:10], '%Y-%m-%d'
                                               ) + datetime.timedelta(days=1)
        else:
            print("Get funding rate error!")
        ...
        print(
            "The newest FR of", timeFR, " is:", newestFR,
    ...
    def crawlAll(
        ...
        self.crawlFR()
        print(
            "Median of FR:", np.median(list(self.fundingRateHistory.values()))
        )
        ...
        for year in range(fromYear, 2020):
            ...
            dayOfMonth = historicalData.dayOfMonthYear if self.isYear(
                year
            ) else historicalData.dayOfMonthNotYear
                            ...
                            while True:
                                ...
                                if len(result
                                    ...
                                    for i in range(0, len(result)):
                                        if (
                                            result[i]['timestamp'] not in
                                            historicalData.recorded[-720:]
                                        ):
                                            fundingRate = self.getFundingRate(
                                                result[i]['timestamp']
                                            )
                                            historicalData.bin1mHistory.append(
                                                list(result[i].values()) +
                                                [float(fundingRate)]
                                            )
                                            historicalData.recorded.append(
                                                result[i]['timestamp']
                                            )
                                        #else:
    ...
    def complement(self, Coin="BTC", filename="BTC.csv", sleep=0.5):
        ...
        lastDateTime = datetime.datetime.strptime(
            lastTimeStamp, "%Y-%m-%dT%H:%M:%S.000Z"
        )
        ...
        for i in range(0, 365 * 1 + 150):
            ...
            for j in range(i * 1440 + 1, (i + 1) * 1440):
                ...
                if lastOpen < lastClose and open > close:
                    ...
                    downDown += 1
            print(
                "upDown:", round(upDown / (upDown + upUp) * 100, 2), "upUp:",
